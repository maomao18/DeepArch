# -*- coding: utf-8 -*-
"""
DA-RNN Baseline — Multi-Seed Training / LSTM基线多种子训练
=========================================================
DA-RNN with input feature attention (Qin et al. 2017) with fixed-feature-conditioned initial state.
...
from models.baselines import DARNNBaseline, count_parameters
from training.baseline_trainer import BaselineTrainer

# ──────────────────────────────────────────────────────────────────
# Model version & seeds
# ──────────────────────────────────────────────────────────────────
MODEL_VERSION = "DA-RNN"
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS):
        ...
        with open(Path(config.log_dir) / "config_used.json", "w", encoding="utf-8") as f:
            save_cfg = {**SHARED_CONFIG, **DARNN_MODEL_CONFIG, "seed": seed, "model": "DARNNBaseline"}
            json.dump(save_cfg, f, cls=_ConfigEncoder, indent=2)

        model = DARNNBaseline(**DARNN_MODEL_CONFIG)
