"""
V2_Homogeneous.py — V2 LHS Data Generation with Homogeneous 1-Layer Material
============================================================================
...
Material: 1-layer homogeneous isotropic material (no gradient, no layering).
  - Exact analytical solution: h = sqrt(12) * ix_target (no iteration).

V2 design:
  - Span L = 1.0 FIXED (not sampled).
  - Lambda is an INDEPENDENT parameter -> direct h and b from lambda_target.
...
Default work_dir: ./V2_Homogeneous_Data
"""
...
DEFAULT_WORK_DIR = r"./V2_Homogeneous_Data"
...
# =============================================================================
# Sample Generation: 1-layer homogeneous (no iteration required)
# =============================================================================


def generate_single_sample(sample_params):
    """
    Process one LHS sample. Homogeneous material -> exact analytical h, b.

    For a homogeneous rectangular cross-section:
        ix = sqrt(I/A) = sqrt(b*h^3/12 / (b*h)) = h / sqrt(12)
    Therefore:
        h = sqrt(12) * ix_target
        b = b_h * h

    No iteration is needed because B11 = 0 (no coupling) and the relationship
    is exact.
    ...
    LHS dimensions (7):
      ...
      5: etaRotL    [0, 1]    left rotational stiffness coefficient
      6: etaRotR    [0, 1]    right rotational stiffness coefficient

    Returns: (is_valid: bool, params: dict)
    """
    (E, rho, b_h, lambda_target, f_L, etaRotL, etaRotR) = sample_params
    ...
    # Exact h from homogeneous ix relation: ix = h / sqrt(12)
    h = math.sqrt(12.0) * ix_target
    b = b_h * h

    # Exact analytical stiffness (no coupling: B11 = 0 for symmetric homogeneous)
    E_pa = E * 1e9
    rho_kgm3 = rho * 1e3

    I0_val = rho_kgm3 * b * h            # mass per unit length [kg/m]
    A11_val = E_pa * b * h                # axial stiffness [N]
    B11_val = 0.0                         # zero coupling for homogeneous
    D11_val = E_pa * b * h**3 / 12.0     # bending stiffness [N.m^2]
    D11_eq = D11_val                      # D11_eq = D11 since B11 = 0

    ix_eq = h / math.sqrt(12.0)
    lambda_real = S / ix_eq
    ...
    # For homogeneous material, lambda_real should match lambda_target exactly
    # (within floating-point precision)
    valid = True

    # Single-layer material lists
    E_list = [E_pa]
    rho_list = [rho_kgm3]
    mu_list = [DEFAULT_MU]
...
def generate_apdl_str(params):
    ...
    return rf"""\
!##############################################################
! V2_Homogeneous — Parabolic Arch Buckling (1-layer, LHS sampling)
! V2 design: L=1.0 fixed, lambda independent, rotational springs only.
! Material: 1-layer homogeneous isotropic (exact analytical h, b).
! Author: Hao Tang
!##############################################################
...
/TITLE,Parabolic Arch Buckling Analysis — V2 Homogeneous
...
def create_commands(work_dir, num_samples=DEFAULT_NUM_SAMPLES):
    """
    1. Generate LHS samples (7-dim).
    2. For each sample, compute homogeneous stiffness analytically.
    ...
    for i in range(num_samples):
        ...
        sample_dir = os.path.join(work_dir, f"Homo_load_disp_{valid_count}")
