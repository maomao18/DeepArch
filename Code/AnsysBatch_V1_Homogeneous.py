"""
V1_Homogeneous.py - V1 data generation with homogeneous 1-layer material
DeepArch Project
Generates ANSYS APDL commands and runs MAPDL simulations in parallel.
Default: 500 samples, output to D:\tanghao\V1_Homogeneous_Data

Usage: python Code/V1_Homogeneous.py
"""
...
def compute_materials(E, rho, mu, b, h):
    """
    Homogeneous 1-layer material computation.
    ...
    I0 = rho_list[0] * b * thickness
    A11 = E_list[0] * b * thickness
    B11 = E_list[0] * b * thickness * z_center
    D11 = E_list[0] * b * (thickness * z_center ** 2 + thickness ** 3 / 12.0)
...
def create_commands(work_dir, num_samples=500, start_idx=0):
    ...
    for i in range(start_idx, start_idx + num_samples):
        ...
        new_dir = os.path.join(work_dir, f"Homogeneous_load_disp_{i}")
...
def main():
    # === Configuration ===
    work_dir = r"./V1_Homogeneous_Data"
    num_samples = 500
    start_idx = 0
