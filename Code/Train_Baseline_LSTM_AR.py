# -*- coding: utf-8 -*-
"""
LSTM Baseline — Multi-Seed Training / LSTM基线多种子训练
=========================================================
Standard stacked LSTM with fixed-feature-conditioned initial state.
...
from models.baselines import LSTMBaseline, count_parameters
from training.baseline_trainer import BaselineTrainer

# ──────────────────────────────────────────────────────────────────
# Model version & seeds
# ──────────────────────────────────────────────────────────────────
MODEL_VERSION = "LSTM_AR"
SEEDS: List[int] = [618, 42, 123, 2024, 9999]  # Full 5 seeds for fair comparison
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS):
        ...
        with open(Path(config.log_dir) / "config_used.json", "w", encoding="utf-8") as f:
            save_cfg = {**SHARED_CONFIG, **LSTM_MODEL_CONFIG, "seed": seed, "model": "LSTMBaseline"}
            json.dump(save_cfg, f, cls=_ConfigEncoder, indent=2)

        model = LSTMBaseline(**LSTM_MODEL_CONFIG)
        print(f"Model params: {count_parameters(model):,}")

        trainer = BaselineTrainer(config, model, best_monitor="val_ar_total_loss")
        trainer.train()
