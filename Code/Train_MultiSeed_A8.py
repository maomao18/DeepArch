# -*- coding: utf-8 -*-
"""
Multi-Seed Training Script — FA-LSTM (Proposed Model) / FA-LSTM多种子训练
==========================================================================
Trains the BucklingPredictor (FA-LSTM) with the exact best config.
Output: models/FA-LSTM/seed_{SEED}/  and  logs/FA-LSTM/seed_{SEED}/

Usage:  python Code/Train_MultiSeed.py
"""
...
# ──────────────────────────────────────────────────────────────────
# Model version & seeds / 模型版本和种子
# ──────────────────────────────────────────────────────────────────
# ══════════════════════════════════════════════════════════════════════════════
# MODEL VERSION — change this to train different variants.
# 改这里即可跑不同消融实验，目录自动区分，不会覆盖。
# ══════════════════════════════════════════════════════════════════════════════
#
#   "FA-LSTM"      ← Full model (all components + scheduled sampling)
#   "FA-LSTM_A1"   ← w/o FeatureAttention + AttnPooling
#   "FA-LSTM_A2"   ← w/o Prior MLP (LSTM initial state)
#   "FA-LSTM_A3"   ← w/o Dual-head decoder
#   "FA-LSTM_A4"   ← w/o Increment loss (w_delta=0, abs-only)
#   "FA-LSTM_A5"   ← w/o Absolute-state loss (w_delta=1, delta-only)
#   "FA-LSTM_A6"   ← w/o FiLM conditioning
#   "FA-LSTM_A7"   ← w/o State Residual
#   "FA-LSTM_A8"   ← w/o Scheduled Sampling (pure TF, 150 epochs)
#                     → change teacher_forcing_epochs=150, transition_epochs=0
#
# For A1-A7: change MODEL_VERSION + the toggle(s) below.
# For A8:     change MODEL_VERSION + teacher_forcing_epochs + transition_epochs.
# ══════════════════════════════════════════════════════════════════════════════

MODEL_VERSION = "FA-LSTM_A8"  # ← CHANGE THIS for ablation
SEEDS: List[int] = [618, 42, 123, 2024, 9999]

# ──────────────────────────────────────────────────────────────────
# FA-LSTM best config (recovered from original checkpoint).
# ──────────────────────────────────────────────────────────────────
# Ablation quick-reference / 消融速查:
#   A1: use_feature_mhsa=False, use_attn_pool=False
#   A2: use_lstm_prior=False
#   A3: use_multihead_outputs=False
#   A4: w_delta=0, w_abs=1.0
#   A5: w_delta=1.0, w_abs=0.0
#   A6: use_film=False
#   A7: use_state_residual=False
#   A8: teacher_forcing_epochs=150, transition_epochs=0  (pure TF, no SS)
# ──────────────────────────────────────────────────────────────────
V2F_CONFIG = dict(
    ...
    teacher_forcing_epochs=150, transition_epochs=0,
    ...
    # ── Ablation toggles (change these for A1-A7) / 消融开关 ──
    use_feature_mhsa=True, use_attn_pool=True, use_lstm_prior=True,
    use_multihead_outputs=True, use_film=True, use_state_residual=True,
...
def _extract_test_metrics(csv_path: str) -> Dict[str, Optional[float]]:
    """Extract test metrics, renaming v2F phys_* columns to match baseline naming."""
    ...
    # Map v2F column names → unified names matching baseline scripts
    RENAME = {
    ...
    for col in df.columns:
        ...
        val = last.get(col)
        if pd.notna(val):
            name = RENAME.get(col, col)  # rename if mapping exists; keep buckling_* as-is
...
def main() -> None:
    ...
    # Unified naming — same as baseline scripts for direct comparison
    key_metrics = [
