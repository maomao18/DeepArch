"""
V2_Compose.py — V2 LHS Data Generation with Composite 5-Layer Material
======================================================================
...
Material: 5-layer composite, each layer with +/-15% random variation
  in E, rho, and mu relative to the sampled baseline.
...
Default work_dir: ./V2_Compose_Data
"""
...
DEFAULT_WORK_DIR = r"./V2_Compose_Data"
DEFAULT_NUM_SAMPLES = 5000
NUM_CORES = "4"
SEED = 42
N_LAYERS = 5
RANDOM_FACTOR_RANGE = (0.85, 1.15)  # +/-15% per layer
MAX_ITER = 20
TOLERANCE = 0.01
...
# =============================================================================
# Sample Generation: 5-layer composite with random variation
# =============================================================================


def generate_single_sample(sample_params, rng):
    """
    ...
    rng: numpy.random.RandomState for reproducible per-layer random factors.
    ...
    # Generate +/-15% random factors for each of the 5 layers
    # Each layer gets an independent random factor applied to E, rho, mu
    factors = rng.uniform(
        RANDOM_FACTOR_RANGE[0], RANDOM_FACTOR_RANGE[1], N_LAYERS
    )
    E_list = factors * E * 1e9
    rho_list = factors * rho * 1e3
    mu_list = np.full(N_LAYERS, DEFAULT_MU) * factors

    # Initial estimate (homogeneous approximation)
    h = math.sqrt(12.0) * ix_target
    b = b_h * h

    # Iterative solve for h, b using 5-layer stiffness
    lambda_real = 0.0
    D11_eq = 0.0
    A11_val = 0.0
    I0_val = 0.0
...
def generate_apdl_str(params):
    ...
    return rf"""\
!##############################################################
! V2_Compose — Parabolic Arch Buckling (5-layer composite, LHS sampling)
! V2 design: L=1.0 fixed, lambda independent, rotational springs only.
! Material: 5-layer composite with +/-15% random per-layer variation.
! Author: Hao Tang
!##############################################################
...
/TITLE,Parabolic Arch Buckling Analysis — V2 Composite
...
def create_commands(work_dir, num_samples=DEFAULT_NUM_SAMPLES):
    ...
    2. For each sample, compute composite stiffness iteratively.
    ...
    # Fixed RNG for reproducible per-layer random factors
    np.random.seed(SEED)
    ...
    for i in range(num_samples):
        sample_idx = i + 1
        # Use a per-sample RNG state derived from the global seed for full reproducibility
        rng = np.random.RandomState(SEED * 10000 + sample_idx)
        valid, params = generate_single_sample(samples[i], rng)
        ...
        sample_dir = os.path.join(work_dir, f"Comp_load_disp_{valid_count}")
