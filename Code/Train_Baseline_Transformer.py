# -*- coding: utf-8 -*-
"""
Transformer Baseline — Multi-Seed Training / Transformer基线多种子训练
=======================================================================
Standard causal Transformer decoder (Vaswani et al. 2017).
Fixed features act as memory tokens for cross-attention.
Teacher forcing only.  No FA, AttnPooling, FiLM, or delta prediction.

Output: models/Transformer/seed_{SEED}/  and  logs/Transformer/seed_{SEED}/

Usage:  python Code/Train_Baseline_Transformer.py
"""
...
from models.baselines import TransformerBaseline, count_parameters
...
def main() -> None:
    ...
    for i, seed in enumerate(SEEDS):
        ...
        with open(Path(config.log_dir) / "config_used.json", "w", encoding="utf-8") as f:
            save_cfg = {**SHARED_CONFIG, **TRANSFORMER_MODEL_CONFIG, "seed": seed, "model": "TransformerBaseline"}
            json.dump(save_cfg, f, cls=_ConfigEncoder, indent=2)

        model = TransformerBaseline(**TRANSFORMER_MODEL_CONFIG)
