"""
V1_FGM.py - V1 data generation with FGM 10-layer material (cos-distribution)
DeepArch Project
Generates ANSYS APDL commands and runs MAPDL simulations in parallel.
Default: 500 samples, output to D:\tanghao\V1_FGM_Data

Usage: python Code/V1_FGM.py
"""
...
def compute_materials(E, rho, mu, b, h):
    """
    FGM 10-layer material computation using cos-distribution.
    Random choice between type1 and type2 FGM gradient profiles.
    Returns: (E_list, rho_list, mu_list, I0, A11, B11, D11, n_layers)
    """
    n_layers = 10
    # Relative z-positions for 10 equally-thick layers (normalized by h)
    z_rel = [-0.45, -0.35, -0.25, -0.15, -0.05, 0.05, 0.15, 0.25, 0.35, 0.45]

    # FGM gradient coefficient and distribution type
    e0 = random.uniform(0.1, 0.3)
    choice = random.choice([1, 2])

    # Compute factorE via cos-distribution
    factorE = []
    for z in z_rel:
        if choice == 1:
            # Type 1: cos(pi * z_rel)
            fe = 1.0 - math.cos(math.pi * z) * e0
        else:
            # Type 2: cos(pi * z_rel / 2 + pi / 4)
            fe = 1.0 - math.cos(math.pi * z / 2.0 + math.pi / 4.0) * e0
        factorE.append(fe)

    # Compute factorRho from factorE:
    #   factorRho = (1 - (1 - factorE))^0.4347826087 * 1.121 - 0.121
    #   simplified to: factorE^0.4347826087 * 1.121 - 0.121
    factorRho = []
    for fe in factorE:
        fr = fe ** 0.4347826087 * 1.121 - 0.121
        factorRho.append(fr)

    # Apply factors to base material properties
    E_list = [fe * E * 1e9 for fe in factorE]
    rho_list = [fr * rho * 1e3 for fr in factorRho]
    mu_list = [mu] * n_layers

    # Compute stiffness properties by integrating over 10 layers
...
! 作者：Hao Tang
! 版本：V1_FGM (10-layer cos-distribution)
! 最后修改：2025-07-27
!##############################################################
...
def create_commands(work_dir, num_samples=500, start_idx=0):
    ...
    for i in range(start_idx, start_idx + num_samples):
        ...
        new_dir = os.path.join(work_dir, f"FGM_load_disp_{i}")
