...
# ══════════════════════════════════════════════════════════════════════════════
#   Paper ablations (matching published results) / 论文消融（匹配发表结果）:
#     "FA-LSTM"      ← Full model
#     "FA-LSTM_A1"   ← w/o FeatureAttention (use_feature_mhsa=False)
#     "FA-LSTM_A2"   ← w/o AttnPooling       (use_attn_pool=False)
#     "FA-LSTM_A3"   ← w/o FiLM               (use_film=False)
#     "FA-LSTM_A4"   ← w/o Prior MLP          (use_lstm_prior=False)
#     "FA-LSTM_A5"   ← w/o State Residual     (use_state_residual=False)
#   Supplementary ablations / 补充消融:
#     "FA-LSTM_S1"   ← w/o Dual-head decoder  (use_multihead_outputs=False)
#     "FA-LSTM_S2"   ← w/o Increment loss     (w_delta=0, w_abs=1.0)
#     "FA-LSTM_S3"   ← w/o Absolute-state loss(w_delta=1.0, w_abs=0.0)
#     "FA-LSTM_S4"   ← w/o Scheduled Sampling (teacher_forcing_epochs=150,
#                                                transition_epochs=0)
# ══════════════════════════════════════════════════════════════════════════════

MODEL_VERSION = "FA-LSTM"  # ← CHANGE THIS for ablation
...
#   A1: use_feature_mhsa=False           (keep use_attn_pool=True)
#   A2: use_attn_pool=False              (keep use_feature_mhsa=True)
#   A3: use_film=False
#   A4: use_lstm_prior=False
#   A5: use_state_residual=False
#   S1: use_multihead_outputs=False
#   S2: w_delta=0, w_abs=1.0
#   S3: w_delta=1.0, w_abs=0.0
#   S4: teacher_forcing_epochs=150, transition_epochs=0
# ──────────────────────────────────────────────────────────────────
V2F_CONFIG = dict(
